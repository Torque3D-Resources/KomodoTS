...
"""TorqueScript support for codeintel.
...
lang = "TorqueScript"
log = logging.getLogger("codeintel.torquescript")
...
# lexers. You must define a Lexer class for lang TorqueScript. If Komodo's
# scintilla lexer for TorqueScript is UDL-based, then this is simply:
#
from codeintel2.udl import UDLLexer
class TorqueScriptLexer(UDLLexer):
    lang = lang
...
# usage.
class TorqueScriptLangIntel(CitadelLangIntel, ParenStyleCalltipIntelMixin,
                   ProgLangTriggerIntelMixin):
    lang = lang
    ...
    def _get_all_known_identifiers(self, buf):
        if TorqueLangIntel._identifier_cplns is None:
            #cplns = [("keyword", x) for x in keywords]
            #cplns += [("function", x) for x in builtins]
            cplns = [] # We want to do this smarter
            TorqueLangIntel._identifier_cplns = sorted(cplns, cmp=CompareNPunctLast, key=operator.itemgetter(1))
        return TorqueLangIntel._identifier_cplns
...
#
class TorqueScriptBuffer(UDLBuffer):
...
# TorqueScript content and returns a description of the code in that file.
# See "cile_torquescript.py" for more details.
#
# The CILE Driver is a class that calls this CILE. If TorqueScript is
...
class TorqueScriptCILEDriver(CILEDriver):
    lang = lang

    def scan_purelang(self, buf):
        import cile_torquescript
        return cile_torquescript.scan_buf(buf)
...
def register(mgr):
    ...
    mgr.set_lang_info(
        lang,
        silvercity_lexer=TorqueScriptLexer(),
        buf_class=TorqueScriptBuffer,
        langintel_class=TorqueScriptLangIntel,
        import_handler_class=None,
        cile_driver_class=TorqueScriptCILEDriver,
